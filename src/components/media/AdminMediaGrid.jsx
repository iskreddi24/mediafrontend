...
//   useEffect(() => {
//     loadData(true);
//   }, [filters.company, filters.mediaType, filters.searchQuery]);
...
//   const handleDeleteClick = async (e, media) => {
//     e.preventDefault();
//     e.stopPropagation();
//     const confirmed = window.confirm(
//       `Are you sure you want to delete "${media.mediaCode}"? This cannot be undone.`
//     );
//     if (!confirmed) return;
...
//           </div>
//           <div className="stat-pill">
//             Showing: <strong>{mediaList.length}</strong>
//           </div>
//         </div>
//         {selectedIds && selectedIds.size > 0 && (
//           <div className="selection-counter">
//             <FaCheck /> <span>Selected: {selectedIds.size}</span>
//           </div>
//         )}
//       </div>
...
//               onClick={() => toggleSelection && toggleSelection(media.id)}
//             >
...
//                     onClick={(e) => handleEditClick(e, media)}
//                   >
//                     <FaEdit />
//                   </button>
//                   <button
...
//                   className="card-img"
//                   onError={(e) => {
//                     if (e.target.src !== LOCAL_PLACEHOLDER) {
...
//                 <div className="media-code-row">
//                   <span className="media-code">{media.mediaCode}</span>
//                 </div>
//                 <div className="location-row">
//                   <FaMapMarkerAlt className="detail-icon" />
//                   <span className="location-text">{media.location}</span>
//                 </div>
//                 <div className="meta-info-grid">
//                   <div className="info-item">
//                     <FaCity className="detail-icon" />
//                     <span className="info-val">{media.city}</span>
//                   </div>
//                   <div className="info-item">
//                     <FaRulerCombined className="detail-icon" />
//                     <span className="info-val">{media.specifications}</span>
//                   </div>
//                 </div>
//               </div>
//             </div>
...
//           <span>Fetching more assets...</span>
//         </div>
//       )}
//     </div>
//   );
...
// Problem 3: image priority helper — imageUrl wins, then imagePath, then placeholder
...
const AdminMediaGrid = ({ filters, selectedIds, toggleSelection }) => {
  ...
  const observer = useRef();
  // Problem 4: guards against the API occasionally re-sending page 0
  // (or a page it already sent) during fast scrolling, which would
  // otherwise create duplicate ids/cards in the grid.
  ...
  const loadData = useCallback(
    async (isNewFilter = false) => {
      ...
      try {
        const data = await getMedia(filters, pageToFetch);
        // Problem 1: API can return content: [null, null, ...] — never trust it raw
        ...
        // Problem 4: de-dupe by id so infinite scroll never renders repeats
        ...
        setMediaList((prev) =>
          pageToFetch === 0 ? freshContent : [...prev, ...freshContent]
        );
        setTotalItems(data.totalElements);
        setHasMore(!data.last);
        if (isNewFilter) setPage(0);
  ...
  const handleEditClick = useCallback((e, media) => {
    e.preventDefault();
    e.stopPropagation();
    setEditingMedia(media);
    setIsEditModalOpen(true);
  ...
  // Problem 2: card click handler is scoped strictly to this media.id.
  // Never toggles based on array index — only ever id-based.
  const handleCardClick = useCallback(
    (media) => {
      if (toggleSelection && media?.id != null) {
    ...
    <div className="media-grid-container">
      ...
      <div className="media-grid">
        {mediaList
          ...
          .map((media, index) => {
            const isLast = index === mediaList.length - 1;
            // Problem 2: selection state is derived purely from media.id
